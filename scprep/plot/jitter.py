...
def jitter(labels, values, sigma=0.1, c=None,title=None, figsize=(8, 6),
    ...
    with temp_fontsize(fontsize):
        ...
        if not labels.shape == values.shape and len(labels.shape) == 1:
            raise ValueError('`labels` and `values` must be 1D arrays of the same length.')
        ...
        # Plotting means
        ax.scatter(np.arange(n_labels), means, c='#cbc9ff', edgecolors='black', lw=1.5, marker='o', zorder=3, s=100)
